...
export default function TermsScreen() {
  ...
  const { fetchLatestTerms, acceptTerms, user } = useAuthStore();
  ...
  const handleScroll = ({ nativeEvent }) => {
    ...
    // Mark as scrolled-to-bottom when within 40px of the end
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      // Route based on next required step
      if (user?.role === "rider" && !user?.documents_uploaded) {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          ...
          <Text style={{ marginTop: SPACING.md, color: COLORS.textMuted }}>Loading Terms...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        {hasContent ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySub}>The platform admin hasn't published rider terms yet. You can proceed for now.</Text>
          </View>
        )}
      </ScrollView>
      ...
      <View style={styles.acceptBar}>
        ...
        <TouchableOpacity
          ...
          onPress={hasContent ? handleAccept : () => router.replace(user?.role === "rider" && !user?.documents_uploaded ? "/(auth)/documents" : "/(tabs)/home")}
        >
        ...
        <Text style={styles.legalLine}>
          By tapping "I Accept", you agree to abide by the WashingBells rider terms.
        </Text>
      </View>
    </SafeAreaView>
  );
